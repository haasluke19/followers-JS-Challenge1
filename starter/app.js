...
async function main() {
  ...
  for (let i = 0; i < NUM_OF_FOLLOWERS; i++) {
    usersPromises.push(fetchUserData(i));
  }

  const users_list = await Promise.all(usersPromises);

  const followersInnerHTML = users_list.map((user) => {
    return (
      `<div class="profile">
  ...
  followersElement.innerHTML = followersInnerHTML.join('');
}
...
const USERS = [
  {
    ...
    avatar: "https://www.gravatar.com/avatar/100?d=identicon&s=64",
